...
export const getParamPerNetwork = <T>(param: iParamsPerNetwork<T>, network: eNetwork) => {
  const { main, goerli, rinkeby, kovan, develop, hardhat, coverage, localhost } = param as iEthereumParamsPerNetwork<T>;
  if (process.env.FORK) {
...
export const getParamPerPool = <T>({ proto }: iParamsPerPool<T>, pool: UnlockdPools) => {
  switch (pool) {
...
export const verifyContract = async (id: string, instance: Contract, args: (string | string[])[]) => {
  ...
  } else if (id == eContractid.UnlockdProxyAdminFund || id == eContractid.UnlockdProxyAdminPool) {
    await verifyEtherscanContract(
      instance.address,
      args,
