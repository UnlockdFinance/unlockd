...
export const deployLockeyManager = async (verify?: boolean) => {
...
export const deployUnlockdProtocolDataProvider = async (addressesProvider: tEthereumAddress, verify?: boolean) =>
  withSaveAndVerify(
    await new UnlockdProtocolDataProviderFactory(await getDeploySigner()).deploy(addressesProvider),
...
export const deployUNFTImplementations = async (
  ...
  for (let x = 0; x < uNftImplementations.length; x++) {
    const uNftAddress = getOptionalParamAddressPerNetwork(poolConfig[uNftImplementations[x].toString()], network);
    if (!notFalsyOrZeroAddress(uNftAddress)) {
      const deployImplementationMethod = chooseUNFTDeployment(uNftImplementations[x]);
...
export const deployUnlockdProxyAdmin = async (id: string, verify?: boolean) =>
  withSaveAndVerify(await new UnlockdProxyAdminFactory(await getDeploySigner()).deploy(), id, [], verify);
...
export const deployUnlockdCollector = async (args: [], verify?: boolean) => {
  const unlockdCollectorImpl = await new UnlockdCollectorFactory(await getDeploySigner()).deploy();
  await insertContractAddressInDb(eContractid.UnlockdCollectorImpl, unlockdCollectorImpl.address);
  return withSaveAndVerify(unlockdCollectorImpl, eContractid.UnlockdCollector, [], verify);
};
