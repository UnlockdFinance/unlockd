...
task("full-upgrade:deploy-unlockd-collector", "Deploy unlockd collect contract")
  ...
  .setAction(async ({ verify, pool }, DRE) => {
    ...
    const initEncodedData = unlockdCollectorImpl.interface.encodeFunctionData("initialize");
...
task("full:upgrade-unlockd-collector", "Upgrade unlockd collect contract")
  ...
  .setAction(async ({ verify, pool, proxy, initFunc }, DRE) => {
    ...
    const unlockdCollectorImpl = await deployUnlockdCollector(verify);
    console.log("Unlockd Collector: new implementation %s", unlockdCollectorImpl.address);
    insertContractAddressInDb(eContractid.UnlockdCollector, unlockdCollectorProxy.address);

    if (initFunc != undefined && initFunc != "") {
      const initEncodedData = unlockdCollectorImpl.interface.encodeFunctionData(initFunc);

      await waitForTx(
        await collectorProxyAdmin
          .connect(proxyAdminOwnerSigner)
          .upgradeAndCall(unlockdCollectorProxy.address, unlockdCollectorImpl.address, initEncodedData)
      );
    } else {
      await waitForTx(
        await collectorProxyAdmin
