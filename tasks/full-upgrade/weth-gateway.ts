...
task(`upgrade:deploy-weth-gateway`, `Deploys the WETHGateway contract`)
  ...
  .setAction(async ({ pool, verify }, DRE) => {
    ...
    console.log("AADDRESSSSS: ", wethGatewayAddress);
...
task("upgrade:wethgateway-authorize-caller-whitelist", "Initialize gateway configuration.")
  .addParam("pool", `Pool name to retrieve configuration, supported: ${Object.values(ConfigNames)}`)
