...
task("upgrade:deploy-proxy-admin", "Deploy proxy admin contract")
  .addParam("pool", `Pool name to retrieve configuration, supported: ${Object.values(ConfigNames)}`)
  .addFlag("skipPool", "Skip proxy admin for POOL")
  .addFlag("skipFund", "Skip proxy admin for FUND")
  .setAction(async ({ pool, skipPool, skipFund }, DRE) => {
    ...
    if (!skipPool) {
      ...
      if (proxyAdminAddress == undefined || !notFalsyOrZeroAddress(proxyAdminAddress)) {
        proxyAdmin = await deployUnlockdProxyAdmin(eContractid.UnlockdProxyAdminPool, false);
      } else {
    ...
    if (!skipFund) {
      ...
      if (proxyAdminAddress == undefined || !notFalsyOrZeroAddress(proxyAdminAddress)) {
        proxyAdmin = await deployUnlockdProxyAdmin(eContractid.UnlockdProxyAdminFund, false);
      } else {
        await insertContractAddressInDb(eContractid.UnlockdProxyAdminFund, proxyAdminAddress);
        proxyAdmin = await getUnlockdProxyAdminByAddress(proxyAdminAddress);
      }
      console.log("UnlockdProxyAdminFund Address:", proxyAdmin.address, "Owner Address:", await proxyAdmin.owner());
    }
  });
