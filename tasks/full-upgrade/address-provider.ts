...
task("upgrade:deploy-address-provider", "Deploy address provider for full enviroment")
  .addParam("pool", `Pool name to retrieve configuration, supported: ${Object.values(ConfigNames)}`)
  .addFlag("skipRegistry")
  .setAction(async ({ pool, skipRegistry }, DRE) => {
    ...
    const addressesProvider = await deployLendPoolAddressesProvider(poolConfig.MarketId, false);
