...
task("verify:general", "Verify general contracts at Etherscan")
  ...
  .setAction(async ({ all, pool }, localDRE) => {
    ...
    const lendPoolConfiguratorProxy = await getUnlockdUpgradeableProxy(lendPoolConfiguratorAddress);
    const lendPoolLoanProxy = await getUnlockdUpgradeableProxy(lendPoolLoanAddress);
    ...
    const proxyAdminFund = await getUnlockdProxyAdminById(eContractid.UnlockdProxyAdminFund);
    await verifyContract(eContractid.UnlockdProxyAdminFund, proxyAdminFund, []);

    const proxyAdminPool = await getUnlockdProxyAdminById(eContractid.UnlockdProxyAdminPool);
    ...
    if (all) {
      ...
      await verifyContract(eContractid.UnlockdProtocolDataProvider, dataProvider, [addressesProvider.address]);
