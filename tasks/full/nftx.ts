...
task("full:deploy-nftx-sushiswap", "Deploy address provider for full enviroment")
  ...
  .setAction(async ({ verify, pool, skipRegistry }, DRE) => {
    ...
    console.log("Setting nftxVault implementation with address:", nftxVaultImpl.address);
    // Set lend pool conf impl to Address Provider
    await waitForTx(await addressesProvider.setNFTXVaultFactory(nftxVaultImpl.address));
    await insertContractAddressInDb(eContractid.NFTXVaultFactory, nftxVaultImpl.address);
    ...
    console.log("Setting nftxVault implementation with address:", sushiSwapImpl.address);
    // Set lend pool conf impl to Address Provider
    await waitForTx(await addressesProvider.setSushiSwapRouter(sushiSwapImpl.address));
    await insertContractAddressInDb(eContractid.SushiSwapRouter, sushiSwapImpl.address);
  });
