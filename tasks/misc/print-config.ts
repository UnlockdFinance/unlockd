...
task("print-config", "Print config of all reserves and nfts")
  ...
  .setAction(async ({ pool }, localBRE) => {
    ...
    const protocolDataProvider = await getUnlockdProtocolDataProvider(await addressesProvider.getUnlockdDataProvider());
    ...
    console.log("UNFT Registry Proxy:", await addressesProvider.getUNFTRegistry());
