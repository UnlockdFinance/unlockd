...
task("dev:initialize-lend-pool", "Initialize lend pool configuration.")
  ...
  .setAction(async ({ verify, pool }, localBRE) => {
    ...
    const dataProvider = await deployUnlockdProtocolDataProvider(addressesProvider.address, verify);
    await insertContractAddressInDb(eContractid.UnlockdProtocolDataProvider, dataProvider.address);
    ...
    await initNftsByHelper(NftsConfig, allNftAddresses, admin, ConfigNames.Unlockd, verify);
    ...
    const unlockdProtocolDataProvider = await deployUnlockdProtocolDataProvider(addressesProvider.address, verify);
