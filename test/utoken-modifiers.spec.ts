...
makeSuite("UToken: Modifiers", (testEnv: TestEnv) => {
  const { CT_CALLER_MUST_BE_LEND_POOL, CALLER_NOT_POOL_ADMIN, CALLER_NOT_UTOKEN_MANAGER } = ProtocolErrors;
  ...
  it("Tries to invoke depositReserves not being the Pool", async () => {
    const { deployer, uWETH } = testEnv;
    await expect(uWETH.depositReserves("1")).to.be.revertedWith(CALLER_NOT_UTOKEN_MANAGER);
  });
  it("Tries to invoke withdrawReserves not being the Pool", async () => {
    const { deployer, uWETH } = testEnv;
    await expect(uWETH.withdrawReserves("1")).to.be.revertedWith(CALLER_NOT_UTOKEN_MANAGER);
  });
