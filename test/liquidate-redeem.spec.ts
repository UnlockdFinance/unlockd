...
import { advanceTimeAndBlock, increaseTime } from "../helpers/misc-utils";
...
makeSuite("LendPool: Redeem", (testEnv) => {
  ...
  it("USDC - Borrows USDC", async () => {
    const { users, pool, reserveOracle, usdc, bayc, configurator, dataProvider, nftOracle, deployer } = testEnv;
    const depositor = users[0];
    const borrower = users[1];
    ...
    await configurator
      .connect(deployer.signer)
      .configureNftAsCollateral(bayc.address, "102", "500000000000000000000", 4000, 7000, 5000, 100, 47, 48, 200, 250);
