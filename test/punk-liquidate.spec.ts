...
makeSuite("PunkGateway-Liquidate", (testEnv: TestEnv) => {
  ...
  before("Initializing configuration", async () => {
    ...
    punkInitPrice = await testEnv.nftOracle.getNFTPrice(testEnv.wrappedPunk.address, testEnv.punkIndexTracker);
  });
  after("Reset", async () => {
    // Reset BigNumber
    BigNumber.config({
    ...
    await setNftAssetPrice(testEnv, "WPUNKS", 101, punkInitPrice.toString());
  });
  ...
  it("Borrow USDC and liquidate it", async () => {
    ...
    await wrappedPunk.connect(liquidator.signer).setApprovalForAll(pool.address, true);
    await waitForTx(await punkGateway.connect(liquidator.signer).auction(punkIndex));
    expect(await wrappedPunk.ownerOf(punkIndex)).to.be.equal(liquidator.address, "Invalid wpunk owner after auction");
  ...
  it("Borrow USDC and redeem it", async () => {
    ...
    await waitForTx(await punkGateway.connect(borrower.signer).redeem(punkIndex, repayAmount));
  ...
  it("Borrow ETH and liquidate it", async () => {
    ...
    await setNftAssetPrice(testEnv, "WPUNKS", punkIndex, punkInitPrice.toString());
  ...
  it("Borrow ETH and redeem it", async () => {
    ...
    const redeemAmountSend = new BigNumber(redeemAmount).toFixed(0);
    await waitForTx(
      await punkGateway.connect(borrower.signer).redeemETH(punkIndex, redeemAmount, {
